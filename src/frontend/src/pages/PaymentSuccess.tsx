...
import { usePaymentSettings, useRemoveAllCartItems } from "../hooks/useQueries";
...
export default function PaymentSuccess() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: clearCart.mutate is stable and should only run once on mount
  useEffect(() => {
      ...
      <Card>
        ...
        <CardContent className="text-center space-y-4">
          ...
          <div className="flex items-start gap-2 p-3 rounded-lg bg-muted/50 text-left text-sm text-muted-foreground">
            ...
            <span>
              Your order includes{" "}
              <strong className="text-foreground">free postage</strong>. Please
              note:{" "}
      ...
      {settingsLoading ? (
        ...
        <Card className="border-primary/30 bg-primary/5">
          ...
          <CardContent className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Your purchase includes complimentary access to the{" "}
              <strong className="text-foreground">
                Pro Ocarina Learning App
              </strong>{" "}
              — learn to read ocarina tablature and compose music.
            </p>
