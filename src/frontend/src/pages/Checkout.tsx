...
import type { DiscountCode, ShoppingItem } from "../backend";
        ...
        checkoutItems = items.map((item) => {
          ...
          const discountedPrice = Math.max(
            ...
            Number(item.priceInCents) -
              Math.round(itemDiscount / Number(item.quantity)),
          );
        ...
        <CardContent>
          <div className="space-y-4">
            {items.map((item, index) => (
              <div
                // biome-ignore lint/suspicious/noArrayIndexKey: cart items use index as stable position key
                key={index}
      ...
      <Alert className="mb-6 border-primary/20 bg-primary/5">
        ...
        <AlertDescription className="space-y-1">
          <p className="font-medium text-foreground">
            ✓ Free postage included on every order
          </p>
          <p className="text-sm text-muted-foreground">
            ✗ Please note: no returns are accepted on any purchases
          </p>
        </AlertDescription>
      </Alert>
