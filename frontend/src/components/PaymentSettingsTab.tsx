...
import { Loader2, Plus, Pencil, Trash2, Tag, Gift, Music } from 'lucide-react';
import { useDiscountCodes, useCreateDiscountCode, useUpdateDiscountCode, useDeleteDiscountCode, usePaymentSettings, useUpdatePaymentSettings } from '../hooks/useQueries';
...
export default function PaymentSettingsTab() {
  ...
  const handleSaveCode = async () => {
    ...
    const numValue = parseFloat(codeForm.value);
    if (isNaN(numValue) || numValue <= 0) {
      setCodeFormError('Value must be a positive number');
      return;
    }
    if (codeForm.discountType === Variant_percentage_fixedAmount.percentage && numValue > 100) {
    ...
    <div className="space-y-8">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <CardDescription>Create and manage promo codes for customers to use at checkout</CardDescription>
              </div>
            </div>
              ...
              <TableBody>
                {discountCodes.map((dc) => (
                  <TableRow key={dc.id}>
                    ...
                    <TableCell>
                      {dc.discountType === Variant_percentage_fixedAmount.percentage
                        ? 'Percentage Off'
                        : 'Fixed Amount'}
                    </TableCell>
                    <TableCell>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                Are you sure you want to delete the code <strong>{dc.code}</strong>? This action cannot be undone.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
        ...
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <CardDescription>Configure a free digital bonus item that appears on every order confirmation</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          {settingsLoading ? (
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                />
                <Label htmlFor="bonus-enabled">Enable bonus item on order confirmations</Label>
              </div>
        ...
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <CardDescription>Set the URL for the Pro Ocarina Learning App — displayed on every order confirmation page</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          {settingsLoading ? (
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-muted-foreground">
                Leave blank to hide the app access section on order confirmations.
              </p>
            </div>
          )}
        </CardContent>
      ...
      <Dialog open={codeDialogOpen} onOpenChange={setCodeDialogOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => setCodeForm((f) => ({ ...f, discountType: v as Variant_percentage_fixedAmount }))}
              >
                <SelectTrigger id="dc-type">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value={Variant_percentage_fixedAmount.percentage}>Percentage Off (%)</SelectItem>
                  <SelectItem value={Variant_percentage_fixedAmount.fixedAmount}>Fixed Amount (AUD)</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="dc-value">
                Value {codeForm.discountType === Variant_percentage_fixedAmount.percentage ? '(%)' : '(AUD)'}
              </Label>
              <Input
                id="dc-value"
                type="number"
                min="0"
                step={codeForm.discountType === Variant_percentage_fixedAmount.percentage ? '1' : '0.01'}
                placeholder={codeForm.discountType === Variant_percentage_fixedAmount.percentage ? 'e.g. 10' : 'e.g. 5.00'}
            ...
            {editingCode && (
              <div className="flex items-center gap-3">
                <Switch
                  ...
                  onCheckedChange={(v) => setCodeForm((f) => ({ ...f, active: v }))}
                />
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setCodeDialogOpen(false)} disabled={isSavingCode}>
              Cancel
            </Button>
            <Button onClick={handleSaveCode} disabled={isSavingCode} className="gap-2">
