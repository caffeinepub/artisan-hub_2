...
import { Music, Play, ShoppingBag, Loader2, Music2 } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Skeleton } from '@/components/ui/skeleton';
import { useGetProducts } from '../hooks/useQueries';
import OcarinaPanel from '../components/OcarinaPanel';
import { playDemo, getScaleFromProduct, SCALE_LABELS, type OcarinaScale } from '../utils/ocarinaSynth';
    ...
    <article className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col">
      ...
      <div className="relative aspect-square overflow-hidden bg-muted">
        ...
        <div className="absolute top-3 left-3">
          <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold border backdrop-blur-sm ${SCALE_BADGE_STYLES[scale]}`}>
      ...
      <div className="p-4 flex flex-col flex-1 gap-3">
        <div>
          ...
          {product.shape && (
            <p className="text-xs text-muted-foreground mt-1 capitalize">{product.shape} · {product.category}</p>
          )}
        ...
        <div className="flex items-center justify-between mt-auto pt-1">
          <span className="font-semibold text-primary text-lg">A${priceAUD}</span>
        </div>
        ...
        <Link
          ...
          className="w-full"
        >
          <Button variant="ghost" size="sm" className="w-full flex items-center gap-1.5 text-xs border border-border/50">
  ...
  return (
    <div className="min-h-screen">
      ...
      <section
        ...
        style={{
          backgroundImage: 'url(/assets/generated/hero-ocarinas.dim_1920x800.png)',
      ...
      >
        ...
        <div className="relative z-10 container max-w-3xl mx-auto">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <span className="text-amber-300 font-semibold tracking-widest uppercase text-sm">Interactive Experience</span>
            <Music className="h-8 w-8 text-amber-300" />
          </div>
          ...
          <p className="text-xl md:text-2xl text-white/90 max-w-2xl mx-auto mb-8">
            Explore our ocarina collection and play each instrument right in your browser — no downloads needed.
          </p>
          <div className="flex flex-wrap items-center justify-center gap-3 text-sm text-white/70">
            {(['bass-low', 'bass', 'alto', 'soprano'] as OcarinaScale[]).map((scale) => (
              <span
                key={scale}
        ...
        <div className="container max-w-4xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-xs text-muted-foreground">Hear a sample melody in the ocarina's pitch range</p>
            </div>
            <div className="flex flex-col items-center gap-2">
              <div className="h-12 w-12 rounded-full bg-primary/10 flex items-center justify-center">
              ...
              <p className="text-xs text-muted-foreground">Open the interactive keyboard and play individual notes</p>
            </div>
            <div className="flex flex-col items-center gap-2">
              <div className="h-12 w-12 rounded-full bg-primary/10 flex items-center justify-center">
              ...
              <p className="text-xs text-muted-foreground">Love the sound? Head to the store to purchase</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="container py-12 px-4">
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-muted-foreground text-lg">Unable to load products. Please try again later.</p>
          </div>
        ) : products.length === 0 ? (
          <div className="text-center py-20">
            <Music className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
          ...
          <>
            <div className="mb-8">
              ...
              <p className="text-muted-foreground">
                {products.length} instrument{products.length !== 1 ? 's' : ''} available — each tuned to its own pitch range
              </p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {products.map((product) => (
                <ProductStudioCard key={product.id.toString()} product={product} />
              ))}
            </div>
          </>
        )}
      ...
      <section className="bg-muted/30 border-t border-border py-16 px-4 text-center">
        <div className="container max-w-2xl mx-auto">
          ...
          <h2 className="font-serif text-3xl font-bold mb-3">Ready to Own Your Ocarina?</h2>
          <p className="text-muted-foreground mb-6">
            Browse our full collection and find the perfect ocarina for your musical journey.
          </p>
          <Link to="/">
